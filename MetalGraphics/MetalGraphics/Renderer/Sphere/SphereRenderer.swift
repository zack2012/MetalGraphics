...
private extension Vertex {
    ...
    func normalize() -> Vertex {
        let position3 = simd_normalize(float3(self.position.x, self.position.y, self.position.z))
        let color = simd_normalize(float3(self.color.x, self.color.y, self.color.z))
        return Vertex(position: float4(position3.x, position3.y, position3.z, 1),
                      color: float4(color.x, color.y, color.z, 1))
    }
}
